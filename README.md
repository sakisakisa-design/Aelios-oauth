...
v1 最终封存点在 tag `v1-final`（当时仍是 MIT）。`tg-bot` 是 Telegram 集成分支，停在 2026-07-16，之后 main 领先它 154 个 commit（135 个文件、约 1.8 万行改动），分支自己只加了 17 个文件，按现状合不回去。留着当历史看；要捡回来得先 rebase 到 main，别直接合。
