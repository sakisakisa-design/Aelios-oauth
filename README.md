...
召回：最后一句用户话 → 向量搜索 + 词面 → 原文片段批量重排 + 规则 → 把干净原文贴到当前消息末尾。默认不需要生成式 LLM，日常最多一条，回答旧事最多两条；低分或重排超时就不注入。分数与取舍可在 `/admin → 设置` 查看，默认阈值需要按实际对话校准。传输信封、哈希和消息 ID 不进入日常提示，同一会话里直接相邻且 90 秒内的两句会合并，整段连续对话不会收成一条；主动搜索仍返回完整记录和 ID，方便删改。日记不自动注入。
