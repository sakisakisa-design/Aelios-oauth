...
相关珍贵原文和召回结果作为一段简短的 markdown 参考追加到当前消息末尾。默认最多 2 条。普通记忆先切成编号窗口（按句子/换行，每段最多 400 字），再注入**存储正文里的那一句**，并标明用途：闲聊是「相关旧事，可自然提及，不作当前事实」（合格集合里只留 1 条），证据问是「回答旧事」（最多 2 条）。空结果合法。提示词里只放正文和用途，不放 memory id、消息 id、信封哈希或内部类型。显式 `memory_search` / `memory_recall` / REST 搜索返回完整记录和 ID，删除、修改、溯源走这条路，不套自动注入的条数预算。原有 system、工具、历史块和 cache_control 不变。
工具续轮不搜索、不恢复旧补丁；下一个人类输入重新召回。没有补丁缓存；请求重试可能再次搜索，但写入按指纹去重。

召回不再整包倾倒珍贵记忆，也不再用末轮短句单独打向量。短句（「那个呢」「继续」）会带上最近几轮用户原文再检索；向量和 D1 词面并行，RRF 合并（LMC-5 / Hindsight 的混合通道）。只有和当前问题词面重叠的珍贵才会进入补丁。自动注入里那句「一句话」是按窗口编号从已存正文摘出来的，不是再生成的摘要；库里若本来就是归档摘要，摘出来的仍可能和最初聊天措辞不同。
