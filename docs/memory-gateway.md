...
相关珍贵原文和召回结果作为一段简短的 markdown 参考追加到当前消息末尾。默认最多 2 条、每条最多 240 字；提示词里只放正文，不放 memory id、消息 id 或内部类型。显式 `memory_search` / REST 搜索仍返回完整记录和 ID，删除、修改、溯源不受影响。原有 system、工具、历史块和 cache_control 不变。
...
原话引用按「可见性」过滤:仍出现在本次请求历史里的原话不参与召回(双方都看得见,召回不增加信息);
上下文压缩把它丢掉后,它自动恢复可召回。规则按内容判,不用时间窗。
客户端消息若带 `<message from="…" msg_id="…">正文</message>` 一类传输信封，只保存和检索正文；旧数据无需先迁移，召回和夜间整理读取时也会清洗。相同会话中，最高分命中只和它时间上直接相邻、且间隔不超过 90 秒的句子合成一个事件；不会顺着 90 秒窗口把整段连续对话收成一条。源消息 ID 仍保留在内部 trace。
