...
- `MEMORY_RERANKER_MODEL`：默认上述 Workers AI 模型，也接受 `workers-ai/@cf/...` 前缀。部署配置已声明 `AI` 绑定；缺失、模型不支持或显式关闭重排时，自动召回本轮为空，不退回未经打分的词面结果。
- `RECALL_RERANK_MIN_SCORE`：默认 **0.5，只是未经线上校准的起点**。分数是该模型的原始相关分数，不是事实正确率；换模型应重新观察分布，不可直接照搬阈值。低分不强取第一条。
- `RECALL_RERANK_TIMEOUT_MS`：默认 1500 毫秒，上限 5000。只限制等待重排的时间，不是整个网关的延迟承诺。异常、无效编号/分数、缺少评分或超时，本轮不注入、不重试、不补调 LLM。Workers AI 绑定不支持此次调用的 AbortSignal，迟到结果会丢弃，但请求可能继续完成并计费。

规则统一处理已在可见历史的片段、重复内容、日常 0–1 条和旧事参考最多 2 条；同空间共享来源 ID 的多条候选本次只呈现一条（可能少给同条消息内的互补事实）。同空间同一 `fact_key` 出现不同正文时，相关分数不能决定哪个版本正确，本轮不选这些候选。来源不足时无法可靠识别所有改写重复；不靠相似度合并或删除库里的记录。近期注入降权和亲笔加成仍影响普通记忆的候选顺序，最终片段按相关分数排序。

短记录不拆句；长记录选连续原文并带前一句，保留局部的说话人、否定和条件。带前句仍超过 400 字的片段会跳过，不切掉句首硬凑；这可能减少长段落召回，也不能保证跨段的限定都被捕获。重排只判断相关性，不校验事实真伪、人物归属或答案是否充分；被选中的计划仍是原文里的计划，不能当作已发生。日记印象不作为事实答案。“最近一次/最新”等需要判定事件顺序的问题默认不自动注入，记录 `latest_requires_evidence`；记录日期不冒充事件日期，主动搜索仍可取完整证据。

**可选实验：**填写 `RECALL_SELECTOR_MODEL`（CF compat 支持的 `author/model`）后，LLM 判断会**替代**以上重排，而非多串一次调用。它沿用面板上游地址、Gateway ID 和 `CLOUDFLARE_API_TOKEN`。模型读取当前发言、最近 3 轮用户上下文（最多 1800 字）、末尾可见历史（最多 6000 字）和编号原文，返回人物/事件/用途判断与窗口编号；程序严格校验后摘原文，不接收生成正文。但编号合法不代表判断正确，因此仍是可选实验。`RECALL_SELECTOR_TIMEOUT_MS` 默认 5000 毫秒、上限 15000，失败或超时不注入。恢复默认须清除面板与 Worker 变量中的该模型设置；面板空值会回落到 Worker 变量。没有真实上游的语义准确率或延迟基准，测试使用固定分数/判断验证程序契约。
...
原话引用只在证据问题（暗号、原话、说过、哪天、最近一次）打开；日常闲聊不扫 messages。所有来源都过滤已在请求历史里的同文窗口，重复候选只保留一份；默认只按明确来源和内容去重；选择实验性 LLM 才增加事件语义判断，不靠相似度删除数据库记录。上下文压缩移除的旧话可以再次召回。库里若本来就是归档摘要，摘出的仍是已存文字，不能恢复最初聊天措辞。

工具续轮不搜索、不恢复旧补丁，下一个人类输入重新判断；原有 system、工具、历史块和 cache_control 不变。判断模型的请求直接走上游，不经过聊天记录入口，不进入 Dream。没有补丁缓存；重试可能再次搜索，写入按指纹去重。
