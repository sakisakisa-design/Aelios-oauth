...
export const SETTINGS: SettingSpec[] = [
  ...
  { group: "模型与线路", name: "CLOAK", label: "Claude OAuth 伪装前缀", hint: "默认开启。填 false 关闭 OAuth 路径的 Claude Code system 前缀注入" },
...
export const SECRET_SPECS: { name: string; label: string }[] = [
  ...
  { name: "CODEX_AUTH_JSON", label: "Codex auth.json 全文（只取其中 refresh_token；与上一条二选一）" },
  { name: "CODEX_ACCOUNT_ID", label: "Codex account_id（留空则从短期票 JWT 里取）" },
